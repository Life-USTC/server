...
<ScrollArea orientation="horizontal">
  ...
  >
    {#if weekdays.length > 0}
      <div role="row" class={cn("grid border-border border-b bg-muted/50 text-center text-muted-foreground text-xs", gridColumns())}>
        {#if showWeekLabels}
          <div role="columnheader" class="border-border border-r px-2 py-2 font-medium">{weekHeaderLabel}</div>
        {/if}
        {#each weekdays as weekday}
          <div role="columnheader" class="px-2 py-2 font-medium">{weekday}</div>
        {/each}
      </div>
    {/if}

    {#each weeks as week}
      <div role="row" class={cn("grid", gridColumns())}>
        {#if showWeekLabels}
          <div role="rowheader" class="flex items-center justify-center border-border border-r border-b bg-muted/35 px-1 py-2 text-center text-muted-foreground text-[0.65rem]">
            {#if week.label}
