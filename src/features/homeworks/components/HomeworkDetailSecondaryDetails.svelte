...
// Three-state status from #1038: sections that do not require completion say so
// instead of reporting a pending/completed state.
$: completionLabel = homework.completionRequired
  ? homework.completed
    ? copy.completedLabel
    : copy.pendingLabel
  : copy.noCompletionRequired;
$: statusValue = [completionLabel].concat(flagLabels).join(" · ");
