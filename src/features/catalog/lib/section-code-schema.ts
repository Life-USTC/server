...
const DELIMITED_SECTION_CODE_TOKEN_SHAPE =
  /^(?=[A-Za-z0-9_.-]*[A-Za-z])(?=[A-Za-z0-9_.-]*\d)(?=[A-Za-z0-9_.-]*[_.-])[A-Za-z0-9_.-]+$/;
const PLAIN_SECTION_CODE_TOKEN_SHAPE =
  /^(?=.*[A-Z])(?=(?:.*[A-Za-z]){2})(?=(?:.*\d){2})[A-Za-z0-9]+$/;
...
export function extractSectionCodeTokens(value: string) {
  ...
  for (const rawToken of value.match(SECTION_CODE_TOKEN_PATTERN) ?? []) {
    ...
    if (
      !DELIMITED_SECTION_CODE_TOKEN_SHAPE.test(token) &&
      !PLAIN_SECTION_CODE_TOKEN_SHAPE.test(token) &&
