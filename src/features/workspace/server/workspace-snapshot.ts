...
import { loadAssistantWorkspaceSnapshotData } from "./workspace-snapshot-queries";

export async function getAssistantWorkspaceSnapshot(input: {
  userId: string;
  locale: AppLocale;
  dayLimit?: number;
  atTime?: Date;
  ...
  const {
    ...
    sectionIds,
    user,
  } = await loadAssistantWorkspaceSnapshotData({
    dateTo,
    locale: input.locale,
    now,
    userId: input.userId,
