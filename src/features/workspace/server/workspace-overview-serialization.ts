import type { OverviewData } from "@/features/workspace/server/workspace-overview-data";
import {
  calendarDateKey,
  workspaceHomeworkItem,
} from "@/features/workspace/server/workspace-page-server";

export function serializeWorkspaceOverview(overview: OverviewData) {
  ...
  const calendarEvents = {
    ...
    semesterHomeworks: overview.semesterHomeworks.map((homework) => ({
      ...workspaceHomeworkItem(homework),
  ...
  return {
    ...
    dueToday: overview.dueToday.map(workspaceHomeworkItem),
    dueWithin3Days: overview.dueWithin3Days.map(workspaceHomeworkItem),
