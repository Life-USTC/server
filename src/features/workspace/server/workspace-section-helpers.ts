...
export const resolveWorkspaceSections = (
  ...
  const workspaceSections = hasCurrentTermSelection ? currentTermSections : [];
  const workspaceSectionIds = Array.from(
    new Set(workspaceSections.map((section) => section.id)),
  );
