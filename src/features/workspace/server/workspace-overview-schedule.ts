...
import { buildRollingCalendarPreview } from "./workspace-overview-calendar";

export function buildWorkspaceOverviewSchedule({
  workspaceSections,
  homeworks,
  locale,
  referenceNow,
}: {
  workspaceSections: Parameters<typeof buildSessions>[0];
  homeworks: Parameters<typeof computeHomeworkBuckets>[0];
  locale: AppLocale;
  referenceNow: ReturnType<typeof getDefaultWeekRange>["start"];
}) {
  const sessions = sortSessionsByStart(buildSessions(workspaceSections));
