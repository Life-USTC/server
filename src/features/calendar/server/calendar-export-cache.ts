...
import { recordCalendarFeedCacheMetric } from "@/lib/metrics/observability-metrics";
...
export async function getCachedUserCalendarExport(
  ...
  if (cached && cached.expiresAtMs > nowMs) {
    recordCalendarFeedCacheMetric({ feed: "user", status: "hit" });
    return {
  ...
  if (!calendar) {
    recordCalendarFeedCacheMetric({ feed: "user", status: "miss" });
    return {
      calendar: null,
