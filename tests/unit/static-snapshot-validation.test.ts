...
describe("static snapshot row validation", () => {
  ...
  it("allows static course rows without assigned teachers", () => {
    const [row] = validateStaticSnapshotRows("courses", [
      {
        id: 1,
        semester_id: "401",
        name: "数学分析",
    ...
    expect(row.teacher_name).toBeNull();
  });

  it("allows static lecture rows without assigned teachers", () => {
    const [row] = validateStaticSnapshotRows("lectures", [
      {
        course_id: 1,
        position: 0,
        start_date: 1_700_000_000,
        end_date: 1_700_003_600,
        name: "数学分析",
        location: "教学楼101",
        teacher_name: null,
        periods: 2,
