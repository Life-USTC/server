...
vi.mock("@/lib/db/prisma", () => ({
  getPrisma: vi.fn(() => ({
    course: { findMany: vi.fn(), findUnique: courseFindUniqueMock },
    section: { findMany: vi.fn(), findUnique: sectionFindUniqueMock },
    teacher: { findMany: vi.fn(), findUnique: teacherFindUniqueMock },
  })),
}));
...
describe("shared public catalog detail cache", () => {
  ...
  it("shares course and teacher detail hits with single-id GraphQL batches", async () => {
    const course = { id: 1, jwId: 101 };
    const teacher = { id: 2, nameCn: "Teacher" };
    ...
    await expect(findCourseDetailByJwId(101, "zh-cn")).resolves.toBe(course);
    await expect(findCoursesByJwIds([101], "zh-cn")).resolves.toEqual([course]);
    await expect(findTeacherDetailById(2, "zh-cn")).resolves.toBe(teacher);
  ...
  it("isolates runtime entries by static import revision", async () => {
    courseFindUniqueMock.mockResolvedValue({ id: 1, jwId: 101 });
