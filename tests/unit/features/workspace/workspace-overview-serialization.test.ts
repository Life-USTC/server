...
import type { OverviewData } from "@/features/workspace/server/workspace-overview-types";
import type {
  ExamItem,
  HomeworkWithSection,
  SessionItem,
} from "@/features/workspace/server/workspace-types";
...
describe("仪表盘概览序列化", () => {
  it("将完整的概览数据序列化为 JSON 友好的结构", () => {
    ...
    const result = serializeWorkspaceOverview(overview);
  ...
  it("overview preview omits semester-wide calendar metadata", () => {
    const result = serializeWorkspaceOverview(
      buildOverviewData({
