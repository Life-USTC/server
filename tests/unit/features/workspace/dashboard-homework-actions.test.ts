...
import type { HomeworkItem } from "@/features/workspace/lib/dashboard-controller-helpers";
...
describe("仪表盘作业操作", () => {
  ...
  it("通过共享客户端更新作业完成状态", async () => {
    ...
    const { toggleDashboardHomeworkCompletion } = await import(
      "@/features/workspace/lib/dashboard-controller-homework-actions"
    );
  ...
  it("展示本地化的仪表盘完成失败信息", async () => {
    ...
    const { createDashboardHomeworkStateActions } = await import(
      "@/features/workspace/lib/dashboard-controller-homework-state-actions"
    );
