...
describe("仪表盘导航统计", () => {
  ...
  it("uses the shared aggregate read model for all navigation counts", async () => {
    ...
    const { getDashboardNavStats } = await import(
      "@/features/workspace/server/dashboard-nav-stats"
    );
  ...
  it("records nav telemetry when the aggregate finishes without waiting for todos", async () => {
    ...
    const { countDashboardStageQuery, createDashboardStageCounter } =
      await import("@/features/workspace/server/dashboard-stage-analytics");
  ...
  it("emits one nav datapoint when the page stage wraps the aggregate", async () => {
    ...
    const { timeDashboardStage } = await import(
      "@/features/workspace/server/dashboard-page-tab-data"
    );
    const { createDashboardStageCounter } = await import(
      "@/features/workspace/server/dashboard-stage-analytics"
    );
    const counter = createDashboardStageCounter({
      dbContext: "rls",
      dbLabel: "app",
