...
test.describe("GET /api/admin/homeworks 作业 moderation 列表", () => {
  ...
  test("管理员可列出作业并包含关键字段", async ({ request }) => {
  ...
  test("管理员可按 status=active 筛选未删除作业", async ({ request }) => {
    await signInAsDevAdminApi(request, "/admin");
    const response = await request.get(`${BASE}?status=active`);
    expect(response.status()).toBe(200);
    const body = (await response.json()) as {
  ...
  test("管理员可按 search 搜索作业标题", async ({ request }) => {
    await signInAsDevAdminApi(request, "/admin");
  ...
  test("管理员可按 search 搜索课程名称", async ({ request }) => {
    await signInAsDevAdminApi(request, "/admin");
    const response = await request.get(
