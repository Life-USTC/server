...
test.describe("PATCH /api/admin/comments/[id] 评论管理", () => {
  ...
  test("空请求体返回 400", async ({ request }) => {
    ...
    const response = await request.patch(`${BASE}/${commentId}`, {
      data: {},
    });
    expect(response.status()).toBe(400);
  });

  test("管理员可管理评论并恢复状态", async ({ request }) => {
    await signInAsDevAdminApi(request, "/admin");
    ...
    try {
      // Softban the comment.
      const softbanResponse = await request.patch(`${BASE}/${commentId}`, {
      ...
      await request.patch(`${BASE}/${commentId}`, {
