...
WHERE NOT EXISTS (
  SELECT 1
  FROM pg_roles
  WHERE rolname = 'life_ustc_runtime'
) \gexec
...
WHERE NOT EXISTS (
  SELECT 1
  FROM pg_roles
  WHERE rolname = 'life_ustc_auth_runtime'
) \gexec
...
WHERE NOT EXISTS (
  SELECT 1
  FROM pg_roles
  WHERE rolname = 'life_ustc_maintenance_runtime'
) \gexec
...
ALTER ROLE life_ustc_auth_runtime
  LOGIN
  PASSWORD 'auth-runtime-test-password'
  NOSUPERUSER
  NOCREATEDB
  NOCREATEROLE
  NOREPLICATION
...
ALTER ROLE life_ustc_maintenance_runtime
  LOGIN
  PASSWORD 'maintenance-runtime-test-password'
  NOSUPERUSER
  NOCREATEDB
  NOCREATEROLE
  NOREPLICATION
...
REVOKE ALL PRIVILEGES
  ON DATABASE :"database_name"
  FROM life_ustc_maintenance_runtime;
...
GRANT UPDATE ("id") ON TABLE "OAuthAccessToken"
TO life_ustc_function_owner;
GRANT UPDATE ("id") ON TABLE "OAuthRefreshToken"
TO life_ustc_function_owner;
GRANT UPDATE ("id") ON TABLE "DeviceCode"
TO life_ustc_function_owner;
GRANT UPDATE ("id") ON TABLE "VerificationToken"
TO life_ustc_function_owner;
GRANT UPDATE ("id") ON TABLE "Session"
TO life_ustc_function_owner;
