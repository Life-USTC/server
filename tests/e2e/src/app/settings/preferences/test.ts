...
test.describe("/settings/preferences 外观与语言偏好", () => {
  test("canonical 路径需要登录", async ({ page }, testInfo) => {
    await expectRequiresSignIn(page, "/settings/preferences");
  ...
  test("legacy query 输入规范到语义路径", async ({ page }) => {
    ...
    const response = await page.request.get("/settings?tab=preferences", {
      maxRedirects: 0,
    });

    expect(response.status()).toBe(308);
    expect(response.headers().location).toBe("/settings/preferences");
  });
